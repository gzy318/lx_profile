    ...
    <body class="text-slate-800 dark:text-slate-200 min-h-screen flex flex-col items-center py-6 px-4">
      ...
      <div class="w-full max-w-[520px] flex justify-between items-center mb-5 gap-2 z-10">
         ...
         <div class="flex gap-2">
            ${music ? `
            <button onclick="toggleMusic()" class="glass-card w-9 h-9 rounded-full flex items-center justify-center hover:scale-110 transition shadow-lg">
               <span id="music-icon">🎵</span><audio id="bg-audio" src="${music}" loop></audio>
            </button>` : ''}
            <button onclick="toggleTheme()" class="glass-card w-9 h-9 rounded-full flex items-center justify-center hover:scale-110 transition shadow-lg">
      ...
      <!-- 2. 公告 -->
      ${notice ? `
      <div class="w-full max-w-[520px] mb-6 glass-card rounded-xl py-2 px-4 overflow-hidden relative z-10 border-l-4 border-blue-500">
         <div class="animate-marquee whitespace-nowrap text-sm font-bold text-blue-600 dark:text-blue-400">
            🔔 ${notice}
         </div>
      </div>` : ''}
      ...
      <main class="w-full max-w-[520px] z-10 animate-[fadeIn_0.5s_ease-out]">
        ...
        <div class="relative mb-5 group">
           <input type="text" id="search-input" placeholder="🔍  搜索链接..." 
                  class="w-full pl-5 pr-4 py-3.5 rounded-2xl glass-card text-sm font-bold outline-none focus:ring-2 focus:ring-blue-500/50 transition border border-white/40 dark:border-white/10 shadow-sm"
                  onkeyup="filterLinks()">
        </div>
        ...
        <footer class="mt-12 text-center pb-12 flex flex-col items-center gap-5">
            <div class="flex flex-wrap justify-center gap-3">
               <div class="footer-pill px-4 py-1.5 rounded-full flex items-center gap-2 text-[10px]">
                  <span>👀</span> <span>${views}</span>
               </div>
               <div class="footer-pill px-4 py-1.5 rounded-full flex items-center gap-2 text-[10px]">
                  <span>⏳</span> <span>${daysRunning} 天</span>
               </div>
               <div class="footer-pill px-4 py-1.5 rounded-full flex items-center gap-2 text-[10px]">
            ...
            <a href="/admin" class="admin-btn px-6 py-2.5 rounded-full text-[10px] uppercase tracking-widest transition transform hover:scale-105">
               进入管理后台
            </a>
        </footer>
      </main>
      ...
      <div id="qr-modal" class="fixed inset-0 bg-black/80 backdrop-blur-sm hidden z-50 flex items-center justify-center" onclick="this.classList.add('hidden')">
         <div class="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-2xl transform scale-95 animate-[fadeIn_0.2s_ease-out]" onclick="event.stopPropagation()">
            ...
            <p class="mt-4 font-bold text-center text-sm dark:text-white">扫一扫分享</p>
         </div>
      </div>
      <div id="toast" class="fixed top-10 left-1/2 -translate-x-1/2 footer-pill px-6 py-2 rounded-full font-bold text-xs shadow-xl transition-all opacity-0 -translate-y-10 z-[100] flex items-center gap-2 border-green-500/50 text-green-400">
        <span>✅</span> 链接已复制
      </div>
      ...
      <script>
        ...
        window.onload = () => {
           ...
           document.getElementById('year-label').innerText = currentYear + '年进度'; // 动态显示年份
...
app.get('/admin', async (c) => {
  ...
  if (cookie !== 'true') return c.html(`
    ...
    <body class="bg-gray-900 h-screen flex items-center justify-center relative overflow-hidden font-sans selection:bg-blue-500 selection:text-white">
      <!-- 动态背景 -->
      <div class="absolute inset-0 z-0">
         <div class="absolute top-0 left-0 w-96 h-96 bg-blue-600/20 rounded-full blur-3xl animate-pulse"></div>
         <div class="absolute bottom-0 right-0 w-96 h-96 bg-purple-600/20 rounded-full blur-3xl animate-pulse" style="animation-delay: 1s;"></div>
      </div>
      ...
      <div class="relative z-10 w-full max-w-sm p-8 bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl animate-[fadeIn_0.5s_ease-out]">
         <div class="text-center mb-8">
            <div class="w-16 h-16 bg-gradient-to-tr from-blue-500 to-purple-500 rounded-2xl mx-auto mb-4 flex items-center justify-center text-3xl shadow-lg">🔐</div>
            <h1 class="text-2xl font-bold text-white tracking-wide">后台管理系统</h1>
            <p class="text-gray-400 text-sm mt-2">请输入管理员密码以继续</p>
         </div>
         
         <form action="/api/login" method="post" class="space-y-6">
            <div class="relative group">
               <input type="password" name="password" required 
                      class="w-full bg-black/20 border border-gray-600 text-white px-4 py-3.5 rounded-xl focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all text-center tracking-widest placeholder-transparent"
                      placeholder="密码">
               <label class="absolute left-1/2 -translate-x-1/2 -top-2.5 bg-[#161b22] px-2 text-xs text-blue-400 transition-all">访问密码</label>
            </div>
            
            <button class="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-blue-900/30 transition transform hover:scale-[1.02] active:scale-95">
               立即解锁
            </button>
         </form>
         
         <div class="mt-8 text-center">
            <a href="/" class="text-xs text-gray-500 hover:text-white transition border-b border-transparent hover:border-gray-500 pb-0.5">返回个人主页</a>
         </div>
      </div>
    </body>
    </html>
    ...
    <body class="bg-[#0b1120] text-slate-300 min-h-screen">
      <nav class="bg-[#1e293b]/80 backdrop-blur border-b border-gray-700 px-6 py-4 flex justify-between items-center sticky top-0 z-50">
         <div class="flex items-center gap-3">
            ...
            <h1 class="font-bold text-white tracking-tight">控制台 <span class="text-[10px] bg-blue-900 text-blue-300 px-1.5 py-0.5 rounded ml-1">V7.0</span></h1>
         </div>
         <a href="/" target="_blank" class="text-sm font-bold text-blue-400 hover:text-white transition flex items-center gap-1 bg-[#0f172a] px-3 py-1.5 rounded-lg border border-gray-700 hover:border-blue-500">
            预览主页 <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
         </a>
      </nav>
        ...
        <div class="lg:col-span-4 space-y-6">
           <div class="bg-[#1e293b] rounded-2xl p-6 shadow-xl border border-gray-700/50">
              ...
              <form action="/api/config" method="post" class="space-y-4">
                 ...
                 <div><label class="text-[10px] font-bold text-gray-500 uppercase">背景图链接 (URL)</label><input type="text" name="bg_url" value="${bgUrl || ''}" class="w-full input-dark p-2.5 rounded-lg text-sm mt-1"></div>
                 <div class="grid grid-cols-2 gap-3">
                    <div><label class="text-[10px] font-bold text-gray-500 uppercase">QQ号</label><input type="text" name="qq" value="${qq || ''}" class="w-full input-dark p-2.5 rounded-lg text-sm mt-1"></div>
                    <div><label class="text-[10px] font-bold text-gray-500 uppercase">建站日期</label><input type="date" name="start_date" value="${startDate || ''}" class="w-full input-dark p-2.5 rounded-lg text-sm mt-1"></div>
                 </div>
                 <div class="grid grid-cols-2 gap-3">
                    <input type="text" name="email" value="${email || ''}" placeholder="电子邮箱" class="input-dark p-2.5 rounded-lg text-sm">
                    <input type="text" name="music_url" value="${music || ''}" placeholder="背景音乐链接 (.mp3)" class="input-dark p-2.5 rounded-lg text-sm">
                 </div>
                 <div class="grid grid-cols-2 gap-3">
                    <input type="text" name="github" value="${github || ''}" placeholder="GitHub 主页" class="input-dark p-2.5 rounded-lg text-sm">
                    <input type="text" name="telegram" value="${telegram || ''}" placeholder="Telegram 链接" class="input-dark p-2.5 rounded-lg text-sm">
                 </div>
        ...
        <div class="lg:col-span-8 space-y-6">
           <div class="bg-[#1e293b] rounded-2xl p-6 shadow-xl border border-gray-700/50 relative overflow-hidden group">
              ...
              <h2 class="text-white font-bold mb-5 flex justify-between items-center text-sm uppercase tracking-wider">
                 <span>${editLink ? '✏️ 编辑模式' : '✨ 添加新链接'}</span>
                 ${editLink ? '<a href="/admin" class="text-xs bg-red-500/10 text-red-400 px-3 py-1 rounded-full border border-red-500/20">取消编辑</a>' : ''}
              </h2>
              <form action="${editLink ? '/api/links/update' : '/api/links'}" method="post" class="space-y-4 relative z-10">
                 ...
                 <div class="grid md:grid-cols-2 gap-4">
                    <input type="text" name="title" value="${editLink?.title || ''}" class="w-full input-dark p-3 rounded-lg" required placeholder="标题 (例如: 我的博客)">
                    <input type="url" name="url" value="${editLink?.url || ''}" class="w-full input-dark p-3 rounded-lg" required placeholder="链接 (https://...)">
                 </div>
                 <div class="grid grid-cols-12 gap-4">
                    <div class="col-span-2"><input type="number" name="sort_order" value="${editLink?.sort_order || 0}" class="w-full input-dark p-3 rounded-lg text-center" placeholder="排序"></div>
                    <div class="col-span-3"><input type="text" name="tag" value="${editLink?.tag || ''}" class="w-full input-dark p-3 rounded-lg" placeholder="标签 (如:工作)"></div>
                    <div class="col-span-7"><input type="text" name="icon" value="${editLink?.icon || ''}" class="w-full input-dark p-3 rounded-lg" placeholder="图标 (Emoji 或 图片URL)"></div>
                 </div>
                 <input type="text" name="description" value="${editLink?.description || ''}" class="w-full input-dark p-3 rounded-lg" placeholder="一句话描述...">
           ...
           <div class="bg-[#1e293b] rounded-2xl shadow-xl border border-gray-700/50 overflow-hidden">
              <div class="p-4 bg-gray-800/50 border-b border-gray-700 flex justify-between items-center">
                 <span class="font-bold text-white text-sm">链接列表 (${linksResult.results.length})</span>
                 <span class="text-[10px] text-gray-500 bg-gray-900 px-2 py-1 rounded">数字越小越靠前</span>
              </div>
                 ...
                 ${linksResult.results.map((link: any) => `
                   <div class="p-4 flex items-center gap-4 hover:bg-gray-700/20 transition group">
                      ...
                      <div class="flex items-center gap-2 opacity-100 lg:opacity-0 group-hover:opacity-100 transition">
                         <a href="/admin?edit_id=${link.id}" class="p-2 bg-blue-500/10 text-blue-400 rounded-lg hover:bg-blue-600 hover:text-white transition">编辑</a>
                         <form action="/api/links/delete" method="post" onsubmit="return confirm('确定要删除吗？')">
                            <input type="hidden" name="id" value="${link.id}">
                            <button class="p-2 bg-red-500/10 text-red-400 rounded-lg hover:bg-red-600 hover:text-white transition">删除</button>
                         </form>
                      </div>
                   </div>
                 `).join('')}
...
// API 逻辑 (保持稳健性)
app.post('/api/login', async (c) => { const body=await c.req.parseBody(); const p=await getConfig(c.env.DB,'password'); if(body.password===p){setCookie(c,'auth','true',{httpOnly:true,maxAge:86400*30,path:'/'});return c.redirect('/admin')}return c.text('密码错误',403)})
