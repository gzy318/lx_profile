/**
 * LX Profile - V21.0 (Fixed & Compact)
 * 1. 修复：彻底解决 "Unexpected end of file" 语法闭合错误。
 * 2. 包含：手机QQ唤起、SSR倒计时、0外部依赖、错误诊断。
 */
...
app.get('/', async (c) => {
  ...
  const tags = ['全部', ...new Set(L.results.map((l:any)=>l.tag?l.tag.trim():'').filter((t:string)=>t!==''))];
  const fv = "https://twbk.cn/wp-content/uploads/2025/12/tx.png";

  return c.html(`<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=0"><title>${ti||'Home'}</title><link rel="icon" href="${fv}"><style>${css}</style><script>if(localStorage.theme==='dark'||(!('theme' in localStorage)&&window.matchMedia('(prefers-color-scheme: dark)').matches))document.documentElement.classList.add('dark');const ps=performance.now();</script></head><body><div class="bg" style="${bg?`background-image:url('${bg}')`:'background-color:#f8fafc'}"></div><div class="w"><div class="top"><div class="pill"><span id="ck">00:00:00</span><span style="opacity:0.2">|</span><span>CN</span></div><div style="display:flex;gap:8px">${mu?`<button class="btn" onclick="pm()" id="mb">🎵<audio id="au" loop></audio></button>`:''}<button class="btn" onclick="tm()">🌗</button></div></div>${nt?`<div class="card" style="padding:10px 16px;border-left:4px solid var(--ac);text-align:left"><div class="mq"><div>🔔 ${nt}</div></div></div>`:''}<div class="card"><img src="/avatar" onerror="this.src='${fv}'" class="av" fetchpriority="high"><h1 class="h1">${ti}</h1><p class="bi" id="bi"></p><div class="soc">${gh?`<a href="${gh}" target="_blank"><svg class="si" viewBox="0 0 24 24"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.065 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.475-1.335-5.475-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z"/></path></svg></a>`:''}${qq?`<a href="javascript:qj()" class="si"><svg class="si" viewBox="0 0 1024 1024"><path d="M824.8 613.2c-16-51.4-34.4-94.6-62.7-165.3C766.5 262.2 689.3 112 511.5 112 331.7 112 256.4 265.2 261 447.9c-28.4 70.8-46.7 113.7-62.7 165.3-34 109.5-23 154.8-14.6 155.8 18 2.2 70.1-82.4 70.1-82.4 0 49 25.2 112.9 79.8 159-26.4 8.1-85.7 29.9-71.6 53.8 11.4 19.3 196.2 12.3 249.5 6.3 53.3 6 238.1 13 249.5-6.3 14.1-23.8-45.2-45.7-71.6-53.8 54.6-46.2 79.8-110.1 79.8-159 0 0 52.1 84.6 70.1 82.4 8.5-1.1 19.5-46.4-14.5-155.8z"/></path></svg></a>`:''}<a href="mailto:${email}" class="em">联系我</a></div><div class="pb"><div class="ph"><span>${y} 余额 ${ld} 天</span><span>${pt}%</span></div><div class="pt"><div class="pf" style="transform:scaleX(${r})"></div></div></div></div><div class="ts">${tags.map((t:string)=>`<div class="tg ${t==='全部'?'a':''}" onclick="fl('${t}',this)">${t}</div>`).join('')}</div><input id="sh" class="ip" placeholder="🔍" onkeyup="sr(this.value)"><div id="ls">${
    // @ts-ignore
    L.results.map((l:any)=>`<a href="${l.url}" target="_blank" class="lk" data-t="${l.tag||''}" data-s="${l.title} ${l.description}"><div class="ic">${!l.icon?`<img src="https://api.iowen.cn/favicon/${new URL(l.url).hostname}.png" loading="lazy">`:(l.icon.startsWith('http')?`<img src="${l.icon}" loading="lazy">`:l.icon)}</div><div class="mn"><div class="tt">${l.title} ${l.tag?`<span class="bd">${l.tag}</span>`:''}</div><div class="ds">${l.description||l.url}</div></div><button class="cp" onclick="cp('${l.url}',event)">📋</button></a>`).join('')}</div><div class="ft"><div class="fi"><span>👀 ${views}</span><span style="opacity:0.3">|</span><span>⏳ ${runDays} D</span><span style="opacity:0.3">|</span><span>⚡ <span id="pf">0</span>ms</span></div><a href="/admin" class="ad">Admin</a></div></div><div id="ts" class="tst">✅ 已复制</div><script>
    document.addEventListener('DOMContentLoaded', () => {
       document.getElementById('pf').innerText=Math.round(performance.now()-ps);
       const ck=document.getElementById('ck');
       function tk(){const d=new Date();const b=new Date(d.getTime()+(d.getTimezoneOffset()*60000)+(3600000*8));ck.innerText=b.getHours().toString().padStart(2,'0')+':'+b.getMinutes().toString().padStart(2,'0')+':'+b.getSeconds().toString().padStart(2,'0');requestAnimationFrame(tk)}requestAnimationFrame(tk);
       ...
       document.addEventListener('error', e=>{if(e.target.tagName==='IMG'&&!e.target.hasAttribute('d')){e.target.setAttribute('d','1');try{e.target.src='https://icons.duckduckgo.com/ip3/'+new URL(e.target.parentNode.parentNode.href).hostname+'.ico'}catch(z){}}},true);
    });
    function qj(){const u="${qq}";if(/Android|iPhone|iPad|iPod/i.test(navigator.userAgent)){window.location.href="mqqapi://card/show_pslcard?src_type=internal&version=1&uin="+u+"&card_type=person&source=sharecard"}else{window.location.href="tencent://AddContact/?fromId=45&subcmd=all&uin="+u}}
    ...
    function tm(){document.body.classList.toggle('dark');document.body.classList.toggle('light')}
